...
function processWeek(wstartdate, wenddate) {
    return new Promise((resolve, reject) => {
        ...
        var query = `SELECT waterconsumption FROM dataset WHERE date BETWEEN "${startdate}" AND "${enddate}";`;
        con.query(query, (err, results) => {
            if (err) {
                ...
                for (var i in eweek) {
                    eweekresult.push(eweek[i].waterconsumption);
                }
...
function processTotal(choice) {
    return new Promise((resolve, reject) => {
        var query;
        if (choice == "This week") {
            query = "SELECT SUM(waterconsumption) AS Total FROM dataset WHERE YEAR(date) = YEAR(CURDATE())  AND WEEK(date) = WEEK(CURDATE());";
        } else if (choice == "Previous week") {
            query = "SELECT SUM(waterconsumption) AS Total FROM dataset WHERE YEAR(date) = YEAR(CURDATE())  AND WEEK(date) = WEEK(CURDATE())-1;";
        } else if (choice == "This Month") {
            query = "SELECT SUM(waterconsumption) AS Total FROM dataset WHERE YEAR(date) = YEAR(CURDATE())  AND Month(date) = Month(CURDATE());";
        } else {
            query = "SELECT SUM(waterconsumption) AS Total FROM dataset WHERE YEAR(date) = YEAR(CURDATE())  AND Month(date) = Month(CURDATE())-1;";
        }
