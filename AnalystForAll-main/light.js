...
function processWeek(wstartdate, wenddate) {
    return new Promise((resolve, reject) => {
        ...
        var query = `SELECT lightsused FROM dataset WHERE date BETWEEN "${startdate}" AND "${enddate}";`;
        con.query(query, (err, results) => {
            if (err) {
                ...
                for (var i in eweek) {
                    eweekresult.push(eweek[i].lightsused);
                }
...
function processTotal(choice) {
    return new Promise((resolve, reject) => {
        var query;
        if (choice == "This week") {
            query = "SELECT SUM(lightsused) AS Total FROM dataset WHERE YEAR(date) = YEAR(CURDATE())  AND WEEK(date) = WEEK(CURDATE());";
        } else if (choice == "Previous week") {
            query = "SELECT SUM(lightsused) AS Total FROM dataset WHERE YEAR(date) = YEAR(CURDATE())  AND WEEK(date) = WEEK(CURDATE())-1;";
        } else if (choice == "This Month") {
            query = "SELECT SUM(lightsused) AS Total FROM dataset WHERE YEAR(date) = YEAR(CURDATE())  AND Month(date) = Month(CURDATE());";
        } else {
            query = "SELECT SUM(lightsused) AS Total FROM dataset WHERE YEAR(date) = YEAR(CURDATE())  AND Month(date) = Month(CURDATE())-1;";
        }
